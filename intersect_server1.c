...
static uint32_t m, L, line, expon, FF_size = 17; //(1ULL << 20ULL) - 1; //n = number of records, m = number of attributes, L = bitlength per attribute, line = record bitlength
...
static int intersect(int n_fixed, int l, int L, int no_multis_bytes, uint8_t (*alt_D)[tobytes(n_fixed)], uint8_t (*T1)[no_multis_bytes], uint8_t **Z1, int *intersect_indices, bool *t){
    ...
    uint8_t (*d1)[nbytes_fixed] = malloc(initsize), (*e1)[nbytes_fixed] = malloc(initsize);
...
static void cout_product_sharing(int batch_bytes, uint8_t (*r)[batch_bytes * 8], uint8_t (*a)[batch_bytes * 8], uint8_t **msgs1, uint8_t *msgs2, int noOTs_bytes){
...
static int couteauHybridET(int n_fixed, int max_depth, uint8_t **Z1, uint8_t (*R)[tobytes(n_fixed)], uint32_t (*A)[n_fixed], int no_multis_bytes, uint8_t (*T1)[no_multis_bytes], int *intersect_indices, bool *t, int *js, int *js_acc){
    ...
    int ret = intersect(n_fixed, rsize, rsize, no_multis_bytes, XS, T1, Z1, intersect_indices, t);
    free(XS);
...
static int generate_dabits(int batch_bytes, uint8_t *B1, uint32_t *A1){
    ...
    if(verbose) printf("finished generating dabits\n");
    return 0;
}
...
static void* client_query_thread(void *arg){
    ...
    uint32_t *dab1A = malloc(nbytes_fixed * 8 * sizeof(uint32_t));
    if(!dab1B || !dab1A){
    ...
    if(cout){
        ...
        if(light){
            if(!benchmarking){
                uint8_t (*R)[nbytes_fixed] = malloc(nbytes_fixed * noOTs_j);
                uint32_t (*A)[nbytes_fixed * 8] = malloc(4 * nbytes_fixed * 8 * noOTs_j);
                if(!R || !A){
                    fprintf(stderr, "could not allocate R or A\n");
                    return NULL;
                }
                int no_multis_bytes = nbytes_fixed * (rsize - 1);
                uint8_t (*T1)[no_multis_bytes] = malloc(no_multis_bytes * 3);
                if(!T1){
            ...
            if(!benchmarking){
                uint8_t (*R)[nbytes_fixed] = malloc(nbytes_fixed * noOTs_j * sizeof(uint8_t));
                uint32_t (*A)[nbytes_fixed * 8] = malloc((8 * nbytes_fixed) * noOTs_j * sizeof(uint32_t));
                if(!R || !A){
                    fprintf(stderr, "could not allocate R or A\n");
                    return NULL;
                }
                uint8_t (*r)[nbytes_fixed] = malloc(nbytes_fixed * expon), (*a)[nbytes_fixed] = malloc(nbytes_fixed * expon);
                if(!r || !a){
                    fprintf(stderr, "could not allocate r or a\n");
                    return NULL;
                }
        ...
        if(!benchmarking){
            ...
            end_time = clock();
            if(intersect(n_fixed, query_len, L, no_multis_bytes, NULL, T1, &Z1, intersection_indices, t) == -1){
